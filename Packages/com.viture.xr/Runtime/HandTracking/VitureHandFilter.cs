...
namespace Viture.XR
{
    ...
    {
        private const int k_HistorySize = 10;
        private const float k_RotationThreshold = 8.0f; // degrees/s
        private const float k_MoveThreshold = 0.002f; // m/s
        
        private const float k_AttackSpeed = 10.0f;
        private const float k_DecaySpeed = 2.0f;
        
        private const float k_PosDeadZoneThreshold = 0.002f;
        private const float k_RotDeadZoneThreshold = 0.2f;
        ...
        internal static void ProcessJoint(Handedness handedness, VitureHandJointID jointId, ref Vector3 position, ref Quaternion rotation)
        {
            ...
            filter.Process(ref position, ref rotation, Time.time);
        }
        ...
        private static JointFilter GetOrCreateJointFilter(Handedness handedness, VitureHandJointID jointId)
        {
            ...
            if (!s_JointFilters.TryGetValue(key, out var filter))
            {
                ...
                var (initialMin, initialBeta) = GetParamsForLevel(s_CurrentMode, level);
                filter = new JointFilter(level, initialMin, initialBeta);
        ...
        private static void RefreshAllParameters()
        {
            foreach (var (key, filter) in s_JointFilters)
            {
                ...
                filter.ForceUpdateParams(minCutoff, beta);
            }
        }
        ...
        {
            private readonly JointLevel m_JointLevel;
            
            private readonly OneEuroFilter m_PosX, m_PosY, m_PosZ;
            private readonly OneEuroFilter m_RotX, m_RotY, m_RotZ, m_RotW;
            
            private readonly Vector3[] m_PosBuffer = new Vector3[k_HistorySize];
            private readonly float[] m_TimeBuffer = new float[k_HistorySize];
            private int m_BufferHead;
            private int m_DataCount;
            private Quaternion m_LastFrameRawRot = Quaternion.identity;

            private float m_CurrentMinCutoff;
            private float m_CurrentBeta;

            private Vector3 m_LastOutputPos;
            private Quaternion m_LastOutputRot;
            private bool m_IsFirstFrame = true;

            private const float k_Frequency = 75f;

            internal JointFilter(JointLevel level, float initialMinCutoff, float initialBeta)
            {
                m_JointLevel = level;
                m_CurrentMinCutoff = initialMinCutoff;
                m_CurrentBeta = initialBeta;
                
                m_PosX = new OneEuroFilter(initialMinCutoff, initialBeta, k_Frequency);
                m_PosY = new OneEuroFilter(initialMinCutoff, initialBeta, k_Frequency);
                m_PosZ = new OneEuroFilter(initialMinCutoff, initialBeta, k_Frequency);
                m_RotX = new OneEuroFilter(initialMinCutoff, initialBeta, k_Frequency);
                m_RotY = new OneEuroFilter(initialMinCutoff, initialBeta, k_Frequency);
                m_RotZ = new OneEuroFilter(initialMinCutoff, initialBeta, k_Frequency);
                m_RotW = new OneEuroFilter(initialMinCutoff, initialBeta, k_Frequency);
            }

            internal void Process(ref Vector3 position, ref Quaternion rotation, float time)
            {
                var isMoving = TryGetAimIntention(position, rotation);

                UpdateDynamicParams(isMoving);
                ...
                var filteredRot = new Quaternion(
                ...
                position = ApplyPositionDeadZone(filteredPos);
                rotation = ApplyRotationDeadZone(filteredRot);
            }

            internal void ForceUpdateParams(float minCutoff, float beta)
            {
                m_CurrentMinCutoff = minCutoff;
                m_CurrentBeta = beta;
                UpdateSubFilters(minCutoff, beta);
            }

            private void UpdateDynamicParams(bool isMoving)
            {
                var (targetMin, targetBeta) = isMoving 
                    ? GetParamsForLevel(VitureHandFilterMode.Responsive, m_JointLevel) 
                    : GetParamsForLevel(VitureHandFilterMode.Stable, m_JointLevel);

                var transitionSpeed = isMoving ? k_AttackSpeed : k_DecaySpeed;
                var dt = Time.deltaTime;

                m_CurrentMinCutoff = Mathf.MoveTowards(m_CurrentMinCutoff, targetMin, transitionSpeed * dt);
                m_CurrentBeta = Mathf.MoveTowards(m_CurrentBeta, targetBeta, transitionSpeed * dt);

                UpdateSubFilters(m_CurrentMinCutoff, m_CurrentBeta);
            }
            
            private void UpdateSubFilters(float minCutoff, float beta)
            {
            ...
            private bool TryGetAimIntention(Vector3 currPos, Quaternion currRot)
            {
                var currTime = Time.time;
                if (m_DataCount == 0)
                {
                    m_BufferHead = 0;
                    m_PosBuffer[0] = currPos;
                    m_TimeBuffer[0] = currTime;
                    m_LastFrameRawRot = currRot;
                    m_DataCount = 1;
                    return false;
                }

                m_BufferHead = (m_BufferHead + 1) % k_HistorySize;
                m_PosBuffer[m_BufferHead] = currPos;
                m_TimeBuffer[m_BufferHead] = currTime;
                if (m_DataCount < k_HistorySize) m_DataCount++;

                var angleDiff = Quaternion.Angle(currRot, m_LastFrameRawRot);
                var angularSpeed = angleDiff / Mathf.Max(Time.deltaTime, 0.0001f);
                m_LastFrameRawRot = currRot;

                if (angularSpeed > k_RotationThreshold) return true;
                if (m_DataCount < 2) return false;

                var totalDist = 0f;
                var tailIndex = (m_BufferHead - m_DataCount + 1 + k_HistorySize) % k_HistorySize;
                for (var i = 0; i < m_DataCount - 1; i++)
                {
                    var idxCurrent = (tailIndex + i) % k_HistorySize;
                    var idxNext = (tailIndex + i + 1) % k_HistorySize;
                    totalDist += Vector3.Distance(m_PosBuffer[idxCurrent], m_PosBuffer[idxNext]);
                }

                var totalTime = m_TimeBuffer[m_BufferHead] - m_TimeBuffer[tailIndex];
                if (totalTime < 0.0001f) return false;
                var linearSpeed = totalDist / totalTime;
                return linearSpeed > k_MoveThreshold;
            }

            private Vector3 ApplyPositionDeadZone(Vector3 inputPosition)
            {
                if (m_IsFirstFrame)
                {
                    m_LastOutputPos = inputPosition;
                    m_IsFirstFrame = false;
                    return inputPosition;
                }
                var distDelta = Vector3.Distance(inputPosition, m_LastOutputPos);
                if (distDelta < k_PosDeadZoneThreshold) return m_LastOutputPos;
                
                var t = k_PosDeadZoneThreshold / distDelta;
                var resultPosition = Vector3.Lerp(inputPosition, m_LastOutputPos, t);
                m_LastOutputPos = resultPosition;
                return resultPosition;
            }

            private Quaternion ApplyRotationDeadZone(Quaternion inputRotation)
            {
                if (m_IsFirstFrame)
                {
                    m_LastOutputRot = inputRotation;
                    m_IsFirstFrame = false;
                    return inputRotation;
                }
                var angleDelta = Quaternion.Angle(inputRotation, m_LastOutputRot);
                if (angleDelta < k_RotDeadZoneThreshold) return m_LastOutputRot;
                
                var t = k_RotDeadZoneThreshold / angleDelta;
                var resultRotation = Quaternion.Slerp(inputRotation, m_LastOutputRot, t);
                m_LastOutputRot = resultRotation;
