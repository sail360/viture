...
namespace Viture.XR.Editor
{
    ...
    class VitureSection : IBuildingBlockSection
    {
        ...
        readonly IBuildingBlock[] m_BBlocksElementIds = new IBuildingBlock[]
        {
            ...
            new VitureRGBCameraBuildingBlock(),
        };
    ...
    class VitureRGBCameraBuildingBlock : IBuildingBlock
    {
        const string k_Id = "RGB Camera API";
        const string k_BuildingBlockPath = VitureEditorUtils.k_BuildingBlockPathO + k_Id;
        const string k_IconPath = "buildingblockIcon";
        const string k_Tooltip = "Adds RGB Camera demo: live feed, frame capture, and resolution control. Requires VITURE glasses with built-in RGB camera.";
        ...
        {
            VitureEditorUtils.s_SDKVersion = VitureConstants.k_DefaultSdkVersion;
            GameObject instance = VitureEditorUtils.CheckAndCreatePrefabForBuildingBlocks(
                VitureEditorUtils.k_SDKPackageName,
                VitureEditorUtils.s_SDKVersion,
                VitureEditorUtils.k_SDKPackageDisplayName,
                VitureEditorUtils.k_SDKSampleRGBCameraDemo,
                VitureEditorUtils.k_VitureRGBCameraPrefabName);

            VitureEditorUtils.DisableNonVitureMainCameras();

            if (instance != null)
            {
                var manager = instance.GetComponentInChildren<VitureRGBCameraManager>(true);
                if (manager != null)
                {
                    manager.StartCameraOnEnable = true;
                    EditorUtility.SetDirty(manager);
                }
            }

            if (EditorBuildSettings.TryGetConfigObject(VitureConstants.k_SettingsKey, out VitureSettings settings) && settings != null)
            {
                settings.CameraPermission = true;
                EditorUtility.SetDirty(settings);
            }

            VitureEditorUtils.EnsureOESBlitInAlwaysIncludedShaders();
        }
